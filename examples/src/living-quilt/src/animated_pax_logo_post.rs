...
const BACKING_BOTTOM_RIGHT: Point = Point::new(100.0, 94.7597);
...
impl Default for AnimatedPaxLogoPost {
    fn default() -> Self {
        ...
        Self {
            ...
            backing_fabric_elements: Property::new(backing_fabric_path(0.0)),
...
fn backing_fabric_path(extension: f64) -> Vec<PathElement> {
    let extension = extension.clamp(0.0, 1.018).max(0.002);
    let bottom_left = TOP_FRONT_RIGHT.lerp(BOTTOM_RIGHT, extension);
    let bottom_right = TOP_BACK_RIGHT.lerp(BACKING_BOTTOM_RIGHT, extension);
...
mod tests {
    ...
    fn post_paths_keep_stable_topologies_at_every_motion_phase() {
        ...
        for controls in samples {
            ...
            assert_eq!(backing_fabric_path(controls.extension).len(), 8);
        }
    }

    #[test]
    fn white_post_face_drops_as_a_rigid_parallelogram() {
        let settled = backing_fabric_path(1.0);
        assert_eq!(settled[0], point_element(TOP_FRONT_RIGHT));
        assert_eq!(settled[2], point_element(TOP_BACK_RIGHT));
        assert_eq!(settled[4], point_element(BACKING_BOTTOM_RIGHT));
        assert_eq!(settled[6], point_element(BOTTOM_RIGHT));
    }
    ...
    fn collapsed_fabric_has_no_pre_fall_sag() {
        ...
        let falling = fabric_path(motion_from_controls(fall_controls(0.5, 0.5, 0.0, 0.0, 3.6)));
