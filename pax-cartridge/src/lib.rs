...
pub fn instantiate_component_stacker<R: 'static + RenderContext>(_instance_registry: Rc<RefCell<InstanceRegistry<R>>>, _args: InstantiationArgs<R>) -> Rc<RefCell<ComponentInstance<R>>>  {
    unreachable!("{}", PLACEHOLDER_ERROR)
}

pub fn instantiate_main_component<R: 'static + RenderContext>(_instance_registry: Rc<RefCell<InstanceRegistry<R>>>) -> Rc<RefCell<ComponentInstance<R>>> {
    unreachable!("{}", PLACEHOLDER_ERROR)
}