    ...
    pub fn to_cursor_style(&self) -> CursorStyle {
        ...
        match self.cursor_type {
            DesignerCursorType::Rotation => {
                let svg = format!(
                    r##"<svg xmlns="http://www.w3.org/2000/svg" width="{CURSOR_SIZE}" height="{CURSOR_SIZE}" viewBox="0 0 256 256">
                        <g transform="rotate({} 128 128)">
                ...
                CursorStyle::Url(
                    format!("data:image/svg+xml,{}", encoded_svg),
                    CURSOR_SIZE / 2,
                    CURSOR_SIZE / 2,
                )
            }
