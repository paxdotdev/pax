...
pub fn slot_dot_control_set(ctx: NodeContext, item: GlassNode) -> ControlPointSet {
    ...
    let slot_dot_control_points = Property::computed(
        move || {
            let mut slots = vec![];
            ...
            slots
                ...
                .map(|s| {
                    ...
                    let t_and_b = slot_child.transform_and_bounds.clone();
                    let t_and_b = t_and_b.get().as_transform();
                    CPoint {
                        point: t_and_b * Point2::new(0.5, 0.5),
    ...
    ControlPointSet {
        ...
        styling: slot_dot_point_styling,
    }
}
