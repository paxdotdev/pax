...
impl<T: Identifier> SetOperations for Subpath<T> {
    fn union(&self, other: &Self) -> Self {
        ...
        let mut all_intersections: Vec<Vec<IntersectionSet>> = Vec::new();
        for (self_ind, self_seg) in self_segments.iter().enumerate() {
            for (other_ind, other_seg) in other_segments.iter().enumerate() {
                let segment_intersections: Vec<_> = intersections(&self_seg, &other_seg, None)
                    .into_iter()
                    .map(|[self_t, other_t]| IntersectionSet {
                        self_t,
                        other_t,
                        self_segment_index: self_ind,
                        other_segment_index: other_ind,
                    })
                    .collect();
                all_intersections.push(segment_intersections);

                // if self_seg.intersections_between_subcurves(, , )
            }
        }
        other.clone()
    }
...
pub fn intersections(slf: &Bezier, other: &Bezier, error: Option<f64>) -> Vec<[f64; 2]> {
    ...
    // MODIFIED: doesn't use min bound, and doesn't sort t values
    unfiltered_intersections(slf, other, error)
}
