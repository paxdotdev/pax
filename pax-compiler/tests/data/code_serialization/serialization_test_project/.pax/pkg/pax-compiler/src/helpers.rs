...
pub struct InlinedTemplateFinder {
    ...
    pending_attribute: Option<(usize, usize, usize, usize, String)>,
}
...
impl<'ast> Visit<'ast> for InlinedTemplateFinder {
    fn visit_item_struct(&mut self, i: &'ast ItemStruct) {
        if let Some((start_line, start_column, end_line, end_column, content)) = self.pending_attribute.take() {
            // Create a new InlinedTemplate with the struct name and stored attribute data
    ...
    fn visit_attribute(&mut self, i: &'ast Attribute) {
        if i.path.is_ident("inlined") {
            ...
            // Store the attribute data temporarily
            self.pending_attribute = Some((
