...
// Given the provided `template_path`, build the template directory
// and return a String path to the temp directory on disk where the template is built
pub fn compile_and_mount_template_directory(template_path: &str) -> String {
    ...
    let mount_path = Path::new(&temp_dir()).join("pax").join(new_uuid);
