        ...
        fn visit_item_struct(&mut self, i: &'ast ItemStruct) {
            ...
            for attr in &i.attrs {
                ...
                if attr.path.is_ident("inlined") {
                    ...
                    let content = 
                        get_substring_by_line_column(&self.file_contents, start_tuple, end_tuple).unwrap().trim_start_matches("(").trim_end_matches(")").to_string();
                    if has_derive_pax {
    ...
    pub fn clear_inlined_template(file_path: &str, pascal_identifier: &str) {
        ...
        for template in finder.templates {
            if template.struct_name == pascal_identifier {
                ...
                modified_content = replace_by_line_column(&modified_content, template.start, template.end, blank_template).unwrap();
            }
        }
