...
- `README.md` for the product-level overview.
...
- `pax-docs/book/src/SUMMARY.md` for the full docs table of contents.
- `examples/src/*` for working patterns. Prefer copying a current example's shape over making up a new style.

Pax work has two layers:

1. `.pax` templates describe UI trees, layout, bindings, events, animation, and style.
2. Rust files own application state, event handlers, data loading, and platform-facing logic.
