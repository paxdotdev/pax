...
impl AddClassRequest {
    pub fn new(containing_component_type_id: TypeId, name: &str) -> Self {
        Self {
            containing_component_type_id,
            class_name: name.to_string(),
        }
...
#[derive(Serialize, Deserialize, Clone)]
pub struct AddClassResponse {
    pub command_id: Option<usize>,
}

impl Request for AddClassRequest {
    type Response = AddClassResponse;
}

impl Response for AddClassResponse {
    fn set_id(&mut self, id: usize) {
        self.command_id = Some(id);
    }
    fn get_id(&self) -> usize {
...
impl Command<AddClassRequest> for AddClassRequest {
    fn execute(&mut self, manifest: &mut PaxManifest) -> Result<AddClassResponse, String> {
        // TODO add class to component
        Ok(todo!())
    }

    fn as_undo_redo(&mut self) -> Option<UndoRedoCommand> {
        Some(UndoRedoCommand::AddClassRequest(Box::new(todo!())))
    }
}
