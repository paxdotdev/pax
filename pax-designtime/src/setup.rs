...
use toml_edit::{Document, Array, Item};
use std::str::FromStr;

pub fn add_additional_dependencies_to_cargo_toml(dest: &PathBuf, pkg: &str) -> Result<(), toml_edit::TomlError> {
    let cargo_toml_path = dest.join("Cargo.toml");
    let content = fs::read_to_string(&cargo_toml_path).expect("Failed to read Cargo.toml");

    let mut doc = content.parse::<Document>().expect("Failed to parse Cargo.toml");

    let pax_designtime_dependency = Item::from_str(r#"{ path = "../pax-designtime", optional = true }"#)?;
    ...
    match pkg {
        ...
        "pax-cartridge" => {
            ...
            doc["dependencies"]["serde_json"] = Item::from_str(r#"{ version = "1.0.95", optional = true }"#)?;
            doc["dependencies"]["include_dir"] = Item::from_str(r#"{ version = "0.7.3", features = ["glob"], optional = true }"#)?;
            doc["dependencies"]["pax-designtime"] = pax_designtime_dependency;
        },
        "pax-designtime" => {
