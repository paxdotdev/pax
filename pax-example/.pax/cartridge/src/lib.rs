...
pub fn instantiate_component_Grids<R: 'static + RenderContext>(instance_registry: Rc<RefCell<InstanceRegistry<R>>>, mut args: InstantiationArgs<R>) -> Rc<RefCell<ComponentInstance<R>>>  {
    args.component_template = Some(Rc::new(RefCell::new(vec![
...
,
    ...
    args.handler_registry = Some(Rc::new(RefCell::new(HandlerRegistry {
        ...
        did_mount_handlers: vec![
                  |properties|{
                      let properties = &mut *properties.as_ref().borrow_mut();
                      let properties = if let PropertiesCoproduct::Grids(p) = properties {p} else {unreachable!()};
    ...
    args.compute_properties_fn = Some(Box::new(|properties, rtc|{
        ...
        let properties = if let PropertiesCoproduct::Grids(p) = properties {p} else {unreachable!()};
            ...
            let new_value = if let TypesCoproduct::VecLABR__crateCOCOgridsCOCORectDefRABR(v) = new_value { v } else { unreachable!() };
            properties.rects.set(new_value);
            }
...
instantiate_component_Grids( Rc::clone(&instance_registry),
 InstantiationArgs {
...
,
    ...
    slot_index: None,
    repeat_source_expression_vec: None,
    repeat_source_expression_range: Some(Box::new(PropertyExpression::new(6))),
    conditional_boolean_expression: None,
    compute_properties_fn: None,
})
,
