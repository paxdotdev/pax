...
pub fn instantiate_expression_table<R: 'static + RenderContext>() -> HashMap<usize, Box<dyn Fn(ExpressionContext<R>) -> TypesCoproduct>> {
    ...
    //Font::local("Esenka","assets/fonts/Esenka.otf",FontStyle::Italic,FontWeight::Normal)
    vtable.insert(9, Box::new(|ec: ExpressionContext<R>| -> TypesCoproduct {
        

        #[allow(unused_parens)]
        TypesCoproduct::__pax_stdCOCOtypesCOCOtextCOCOFont(
            Font::local(("Esenka".try_into().unwrap()),("assets/fonts/Esenka.otf".try_into().unwrap()),(FontStyle :: Italic.try_into().unwrap()),(FontWeight :: Normal.try_into().unwrap()),)
        )
    }));
...
pax_std_primitives::text::TextInstance::instantiate(
 InstantiationArgs {
    properties: PropertiesCoproduct::Text( Text {
            ...
            font: Box::new( PropertyExpression::new(9) ),
