...
use pax_example::pax_reexports::pax_std::types::Alignment;

use pax_example::pax_reexports::pax_std::types::BoundingBox;
...
pub fn instantiate_expression_table<R: 'static + RenderContext>() -> HashMap<usize, Box<dyn Fn(ExpressionContext<R>) -> TypesCoproduct>> {
    ...
    //Color::rgba(0.3,0.3,1.0,0.4)
    vtable.insert(12, Box::new(|ec: ExpressionContext<R>| -> TypesCoproduct {
        

        #[allow(unused_parens)]
        TypesCoproduct::__pax_stdCOCOtypesCOCOColor(
            Color::rgba((Numeric::from(0.3)),(Numeric::from(0.3)),(Numeric::from(1.0)),(Numeric::from(0.4)),)
        )
    }));
...
pax_std_primitives::text::TextInstance::instantiate(
 InstantiationArgs {
    properties: PropertiesCoproduct::Text( Text {
            ...
            text: Box::new( PropertyLiteral::new("Lorem ipsum dolor sit amet Lorem ipsum dolor sit amet  \nconsectetur adipiscing elit  \nconsectetur adipiscing elit".try_into().unwrap()) ),
        
            font: Box::new( PropertyExpression::new(8) ),
        
            fill: Box::new( PropertyExpression::new(10) ),
        
            paragraph_alignment: Box::new( PropertyLiteral::new(Alignment :: Left.try_into().unwrap()) ),
        
            vertical_alignment: Box::new( PropertyLiteral::new(VAlignment
        :: Bottom.try_into().unwrap()) ),
        
            horizontal_alignment: Box::new( PropertyLiteral::new(Alignment :: Right.try_into().unwrap()) ),
        
            bounding_box: Box::new( PropertyLiteral::new(BoundingBox :: Fixed
    .try_into().unwrap()) ),
