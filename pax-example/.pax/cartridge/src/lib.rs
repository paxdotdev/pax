...
use pax_example::pax_reexports::PaxExample;
...
pub fn instantiate_root_component<R: 'static + RenderContext>(instance_registry: Rc<RefCell<InstanceRegistry<R>>>) -> Rc<RefCell<ComponentInstance<R>>> {
    ComponentInstance::instantiate( InstantiationArgs{
        properties: PropertiesCoproduct::PaxExample( PaxExample::default() ),
                                                     ...
                                                     HandlerRegistry {
                                                         ...
                                                         will_render_handlers: vec![
                                                                     |properties, args|{
                                                                         ...
                                                                         let properties = if let PropertiesCoproduct::PaxExample(p) = properties {p} else {unreachable!()};
                                                                         PaxExample::handle_will_render(properties,args);
                                                                     },
                                                                 ],
                                                         did_mount_handlers: vec![
                                                                      |properties|{
                                                                          let properties = &mut *properties.as_ref().borrow_mut();
                                                                          let properties = if let PropertiesCoproduct::PaxExample(p) = properties {p} else {unreachable!()};
                                                                          PaxExample::handle_did_mount(properties);
                                                                      },
                                                                  ],
                                                         scroll_handlers: vec![],
                                                     }
    ...
    handler_registry:  Some(Rc::new(RefCell::new(
                                                 HandlerRegistry {
                                                     ...
                                                     scroll_handlers: vec![|stack_frame, args|{
                                                                     ...
                                                                     let properties = &mut *properties.as_ref().borrow_mut();
                                                                     let properties = if let PropertiesCoproduct::PaxExample(p) = properties {p} else {unreachable!()};
                                                                     PaxExample::handle_scroll(properties,args);
                                                                 },],
                                                 }
                                             ))),
    instance_registry: Rc::clone(&instance_registry),
